...
@onready var punto: Control = $Control/Panel/punto
@onready var win_label: Label = $Control/Panel/Ganaste
@onready var timer_label: Label = $Control/Panel/TimerLabel
...
func _ready():
	...
	tween.tween_property(ui_root, "modulate:a", 1.0, 0.8).set_trans(Tween.TRANS_SINE).set_ease(Tween.EASE_OUT)
...
func random_move():
	...
	for dir in directions:
		...
		if new_pos.x >= 0 and new_pos.x < cols and new_pos.y >= 0 and new_pos.y < rows:
			...
			if piece:
				piece["tex_rect"].position = punto.position + Vector2(empty_pos.x, empty_pos.y) * piece_size
...
func check_if_solved():
	...
	for piece in pieces:
		var expected_pos = punto.position + Vector2(piece["original_pos"].x, piece["original_pos"].y) * piece_size
